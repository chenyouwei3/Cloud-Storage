...
var (
	...
	cacheCloudStorageUserUserIdPrefix = "cache:cloudStorage:user:userId:"
)
...
func (m *defaultUserModel) Delete(ctx context.Context, userId int64) error {
	cloudStorageUserUserIdKey := fmt.Sprintf("%s%v", cacheCloudStorageUserUserIdPrefix, userId)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `user_id` = ?", m.table)
		return conn.ExecCtx(ctx, query, userId)
	}, cloudStorageUserUserIdKey)
	return err
}
...
func (m *defaultUserModel) FindOne(ctx context.Context, userId int64) (*User, error) {
	cloudStorageUserUserIdKey := fmt.Sprintf("%s%v", cacheCloudStorageUserUserIdPrefix, userId)
	var resp User
	err := m.QueryRowCtx(ctx, &resp, cloudStorageUserUserIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultUserModel) Insert(ctx context.Context, data *User) (sql.Result, error) {
	cloudStorageUserUserIdKey := fmt.Sprintf("%s%v", cacheCloudStorageUserUserIdPrefix, data.UserId)
...
func (m *defaultUserModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheCloudStorageUserUserIdPrefix, primary)
}
