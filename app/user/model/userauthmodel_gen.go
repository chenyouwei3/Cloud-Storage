...
var (
	...
	cacheCloudStorageUserAuthIdPrefix = "cache:cloudStorage:userAuth:id:"
)
...
func (m *defaultUserAuthModel) Delete(ctx context.Context, id int64) error {
	cloudStorageUserAuthIdKey := fmt.Sprintf("%s%v", cacheCloudStorageUserAuthIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, cloudStorageUserAuthIdKey)
	return err
}
...
func (m *defaultUserAuthModel) FindOne(ctx context.Context, id int64) (*UserAuth, error) {
	cloudStorageUserAuthIdKey := fmt.Sprintf("%s%v", cacheCloudStorageUserAuthIdPrefix, id)
	var resp UserAuth
	err := m.QueryRowCtx(ctx, &resp, cloudStorageUserAuthIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultUserAuthModel) Insert(ctx context.Context, data *UserAuth) (sql.Result, error) {
	cloudStorageUserAuthIdKey := fmt.Sprintf("%s%v", cacheCloudStorageUserAuthIdPrefix, data.Id)
...
func (m *defaultUserAuthModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheCloudStorageUserAuthIdPrefix, primary)
}
